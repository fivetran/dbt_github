...
select
  ...
  {{ dbt_utils.datediff('time_of_first_request', "coalesce(time_of_first_review_post_request, current_timestamp)", 'second') }}/3600 as hours_request_review_to_first_review,
  {{ dbt_utils.datediff('time_of_first_request', 
                        "least(coalesce(time_of_first_requested_reviewer_review, current_timestamp()),
                              coalesce(issue.closed_at, current_timestamp()))", 'second') }}/3600 as hours_request_review_to_first_action,
  {{ dbt_utils.datediff('time_of_first_request', 'merged_at', 'second') }}/3600 as hours_request_review_to_merge
