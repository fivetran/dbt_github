...
-- this test ensures the github__weekly_metrics end model matches the prior version
with prod as (
    select 
        week,
        repository,
    ...
    from {{ target.schema }}_github_prod.github__weekly_metrics
    where date(week) < current_date() -- BQ 
    group by 1, 2
),
...
dev as (
    ...
    from {{ target.schema }}_github_dev.github__weekly_metrics
    where date(week) < current_date() -- BQ
    group by 1, 2
),
...
final as (
    select 
        coalesce(prod.week, dev.week) as week,
